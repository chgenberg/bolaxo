  ...
  return (
    <div className="bg-white p-8 rounded-2xl shadow-card">
      <div className="flex items-center justify-between mb-6">
        <h2 className="heading-2">Pipeline Översikt</h2>
        <div className="text-right">
          <p className="text-2xl font-bold">{totalValue}M SEK</p>
          <p className="text-sm text-gray-600">Total pipeline-värde</p>
        </div>
      </div>
          ...
          stage.deals.length > 0 && (
            <div key={stage.name} className="border rounded-lg p-4">
              <div className="flex items-center justify-between mb-3">
                ...
                <span className="px-2 py-1 bg-gray-100 text-gray-700 rounded text-sm">{stage.count} affärer</span>
              </div>
                ...
                {stage.deals.map((deal) => (
                  <div key={deal.id} className="flex items-center justify-between p-2 bg-gray-50 rounded">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      {deal.risk && (
                        <span className={`px-2 py-1 rounded text-xs ${getRiskColor(deal.risk)} flex items-center gap-1`}>
